...
class TicketReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for list_dict in list_dicts:
            ...
            control_report_worksheet.write(row, 2, res_user_obj.browse(list_dict['user_name_id']).name,
                                           cell_wrap_format)

            control_report_worksheet.write(row, 3, user_ticket_group_obj.sudo().browse(
                list_dict['user_ticket_group_id']).name or '', cell_wrap_format)
            control_report_worksheet.write(row, 4, localize_tz(
            ...
            control_report_worksheet.write(row, 5, localize_tz(
                ...
                user_tz_obj).strftime('%d/%m/%Y %I:%M:%S %p') if list_dict['done_ticket_date'] else '',
                                           cell_wrap_format)
            control_report_worksheet.write(row, 6, (
            ...
            control_report_worksheet.write(row, 10,
                                           res_partner_obj.sudo().browse(list_dict['partner_id']).product_id.name or '',
                                           cell_wrap_format)

            control_report_worksheet.write(row, 11, res_partner_obj.sudo().browse(list_dict['partner_id']).name or '',
                                           cell_wrap_format)
            control_report_worksheet.write(row, 12, res_partner_obj.sudo().browse(list_dict['partner_id']).ref or '',
                                           cell_wrap_format)
            control_report_worksheet.write(row, 13, res_partner_obj.sudo().browse(list_dict['partner_id']).street or '',
                                           cell_wrap_format)
            control_report_worksheet.write(row, 14, res_partner_obj.sudo().browse(list_dict['partner_id']).phone or '',
                                           cell_wrap_format)
            control_report_worksheet.write(row, 15, res_partner_obj.sudo().browse(list_dict['partner_id']).mobile or '',
                                           cell_wrap_format)
            control_report_worksheet.write(row, 16, res_partner_obj.sudo().browse(list_dict['partner_id']).email or '',
                                           cell_wrap_format)
            ...
            control_report_worksheet.write(row, 19,
                                           prospect_area_obj.sudo().browse(list_dict['prospect_area_id']).name or '',
                                           cell_wrap_format)
            ...
            control_report_worksheet.write(row, 23, complaint_type_obj.sudo().browse(
                list_dict['complaint_type_support_id']).name or '', cell_wrap_format)
            control_report_worksheet.write(row, 24, list_dict['source_support'], cell_wrap_format)

            control_report_worksheet.write(row, 25,
                                           localize_tz(list_dict['last_log_datetime'], '%Y-%m-%d %H:%M:%S').astimezone(
                                               user_tz_obj).strftime('%d/%m/%Y %I:%M:%S %p') if list_dict[
                                               'last_log_datetime'] else '', cell_wrap_format)
            control_report_worksheet.write(row, 26, res_user_obj.browse(list_dict['last_log_user_id']).name or '',
                                           cell_wrap_format)
