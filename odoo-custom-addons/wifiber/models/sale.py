...
class SaleOrderExtend(models.Model):
    ...
    def action_confirm(self):
        ...
        grp_name = 'wifiber.group_receive_quotation_confirmed_email_to_approve'
    ...
    def action_create_payment_entry(self):
        ...
        action = self.env['ir.model.data'].xmlid_to_object('wifiber.action_create_payment')
        form_view_id = model_data_obj.xmlid_to_res_id('wifiber.create_payment_wizard_view')
    ...
    def create_ticket_with_email(self):
        ...
        category_id = self.env.ref('wifiber.installation')
...
class ResPartnerExtend(models.Model):
    ...
    def action_create_customer_selfcare(self):
        ...
        try:
            response = requests.post("http://api.wifiber.ng:8010/api/create-client", data=payload)
            if response.status_code != requests.codes.ok:
                ...
                # send email
                grp_name = 'wifiber.group_receive_push_selfcare_customer_email'
    ...
    def action_update_customer_selfcare(self):
        ...
        payload = self._get_self_care_payload()
        try:
            response = requests.post("http://api.wifiber.ng:8010/api/update-client", data=payload)
            if response.status_code != requests.codes.ok:
                raise UserError(response.text)
            else:
                self.env.cr.execute("update res_partner set selfcare_push = True, selfcare_response = '%s' where id = %s" % (response.text, self.id))
    ...
    def action_delete_customer_selfcare(self):
        ...
        try:
            response = requests.post("http://api.wifiber.ng:8010/api/delete-client", data=payload)
            if response.status_code != requests.codes.ok:
                raise UserError(response.text)
            else:
                # send email
...
class ProductTemplateExtend(models.Model):
    ...
    is_pick_default = fields.Boolean(string='Pick by Default for Survey', track_visibility='always')
    is_pick_default_qty = fields.Float(string='Pick by Default Qty.', track_visibility='always')
