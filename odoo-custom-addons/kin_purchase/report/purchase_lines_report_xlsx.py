...
class PurchaseReport(models.TransientModel):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for product_id in product_ids :
            ...
            report_worksheet.write_row(row, col, ('S/N', 'Order ID' , 'Order Date', 'Supplier', 'Vendor Ref.' , 'Product', 'Ordered Qty.', 'Received Qty.', 'Billed', 'Un-Billed' ,'Unit Price' ,'Sub-total', 'Status' ) , head_format)
            row += 1
            for list_dict in list_dicts:
                if list_dict['product_id'] == product_id :
                    ...
                    report_worksheet.write(row, 3, list_dict['supplier_name'], cell_wrap_format)
                    report_worksheet.write(row, 4, list_dict['partner_ref'], cell_wrap_format)
                    ...
                    report_worksheet.write(row, 7, list_dict['qty_received'], cell_number)
                    ...
                    report_worksheet.write(row, 11, list_dict['price_subtotal'], cell_amount)
                    report_worksheet.write(row, 12, list_dict['state'], cell_wrap_format)
                    row += 1
            row += 1
        return
