...
class ResConfigSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param(
            'is_select_sales_team', self.is_select_sales_team)
        self.env['ir.config_parameter'].sudo().set_param(
            'is_contraint_sales_order_stock', self.is_contraint_sales_order_stock)
        self.env['ir.config_parameter'].sudo().set_param(
            'is_sales_order_stock_notification', self.is_sales_order_stock_notification)
        self.env['ir.config_parameter'].sudo().set_param(
            'is_send_stock_notification', self.is_send_stock_notification)
        self.env['ir.config_parameter'].sudo().set_param(
            'is_sales_order_stock_count_error', self.is_sales_order_stock_count_error)
        self.env['ir.config_parameter'].sudo().set_param(
            'is_invoice_before_delivery', self.is_invoice_before_delivery)
        self.env['ir.config_parameter'].sudo().set_param(
            'is_post_invoice_before_delivery', self.is_post_invoice_before_delivery)
        self.env['ir.config_parameter'].sudo().set_param(
            'is_send_invoice_notification', self.is_send_invoice_notification)
        self.env['ir.config_parameter'].sudo().set_param(
            'is_sales_order_stock_purchase_request', self.is_sales_order_stock_purchase_request)
        self.env['ir.config_parameter'].sudo().set_param(
            'is_po_check', self.is_po_check)
        self.env['ir.config_parameter'].sudo().set_param(
            'validity_date_interval', self.validity_date_interval)
        self.env['ir.config_parameter'].sudo().set_param(
            'is_send_expiry_email_quote_notification', self.is_send_expiry_email_quote_notification)
        self.env['ir.config_parameter'].sudo().set_param(
            'is_delete_quote_after_expiration_date', self.is_delete_quote_after_expiration_date)
