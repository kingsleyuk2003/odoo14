...
class ActivityStatement(models.AbstractModel):
    ...
    def _display_lines_sql_q1(self, partners, date_start, date_end, account_type):
        return str(
            self._cr.mogrify(
                """
            SELECT m.id as m_id, m.name AS move_id, l.partner_id, l.date,
    ...
    def _display_lines_sql_q2(self, company_id):
        return str(
            self._cr.mogrify(
                """
            SELECT Q1.partner_id, Q1.m_id, Q1.move_id, Q1.date, Q1.date_maturity,
    ...
    def _get_account_display_lines(
        ...
        WITH Q1 AS (%s),
             Q2 AS (%s)
        SELECT m_id, partner_id, move_id, date, date_maturity, name, ref, debit,
                            credit, amount, blocked, currency_id
        FROM Q2
        ORDER BY date, date_maturity, move_id, m_id"""
            % (
